...
public class DashboardController1 implements Initializable {
    ...
    private int getUserIdFromUsersTable() throws SQLException {
        ...
        try {
            ...
            String query = "SELECT user_id FROM service_providers WHERE username=?"; // Replace <criteria> with the appropriate condition
    ...
    private String getUserIdFromUsersTableByEmail() throws SQLException {
        ...
        try {
            ...
            String query = "SELECT email FROM service_providers WHERE username=?";
    ...
    public void emergency_services() throws IOException {
        Parent root = FXMLLoader.load(getClass().getResource("emergency1.fxml"));
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        connect = connectDB();
        try {
            // Define the SQL query to check if data exists in the "req" column of the "request" table
            String checkRequestQuery = "SELECT req FROM request";

            // Create a PreparedStatement to execute the SQL query
            prepare = connect.prepareStatement(checkRequestQuery);
            ...
            // Check if the ResultSet contains any rows
            if (result.next()) {
                // Data exists in the "req" column of the "request" table
                // Show an alert message indicating that there is data in the table
                alertMessage alert = new alertMessage();
                alert.infoMessage("There are pending requests.");
            }
        } catch (SQLException e) {
            e.printStackTrace(); // Print any exceptions that occur during execution
        }
        try {
            // Define the SQL query to check if data exists in the "req" column of the "request" table
            String checkRequestQuery = "SELECT mess FROM message";
            ...
            if (result.next()) {
                ...
                // Show an alert message indicating that there is data in the table
                alertMessage alert = new alertMessage();
                alert.infoMessage("Someone wants to message you!");
            }
        } catch (SQLException e) {
            e.printStackTrace(); // Print any exceptions that occur during execution
        }
